...
<form action="<?php echo JRoute::_('index.php?option=com_users&view=users');?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		...
		<div class="filter-select fltrt">
			...
			<select name="filter_rangegroup" id="filter_rangegroup" class="inputbox" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('COM_USERS_OPTION_FILTER_GROUP');?></option>
				<?php echo JHtml::_('select.options', Usershelper::getRangeGroup(), 'value', 'text', $this->state->get('filter.rangegroup'));?>
			</select>
			<?php
           $custom_groupId = $this->state->get('filter.rangegroup');
           
			if($custom_groupId == 9){?>
			<select name="filter_auditorfirm" id="filter_auditorfirm" class="inputbox" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('COM_USERS_OPTION_FILTER_AUDITOR_FIRM');?></option>
				<?php echo JHtml::_('select.options', Usershelper::getAuditorFirm(), 'value', 'text', $this->state->get('filter.auditorfirm'));?>
			</select>
			<?php } elseif($custom_groupId == 11) {?>
			<select name="filter_lawfirm" id="filter_lawfirm" class="inputbox" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('COM_USERS_OPTION_FILTER_LAWFIRM');?></option>
				<?php echo JHtml::_('select.options', Usershelper::getLawFirm(), 'value', 'text', $this->state->get('filter.lawfirm'));?>
			</select>
			<?php } ?>
		</div>
	</fieldset>
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th class="nowrap" width="10%">
					<?php echo JHtml::_('grid.sort', 'View Detail', 'a.username', $listDirn, $listOrder); ?>
				</th>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="center">
					<?php echo $this->escape($item->username); ?>
								...
								<td class="center">
					<a href="<?php echo JRoute::_('index.php?option=com_users&view=users&layout=userdetail&tmpl=component&id='.(int) $item->id);?>" class="modal" rel="{handler: 'iframe', size: {x: 700}}"><?php echo "View Detail" ?></a>
				</td>
				<td class="center">
					<?php if ($canChange) : ?>
