...
class UsersHelper
{
	...
	/**
	 * Get a list of filter options for the custom user group.
	 * @return mixed
	 */
     static function getRangeGroup()
	{
		// Build the filter options.
		$options = array();
		$options[] = JHtml::_('select.option', '9', 'Auditor');
		$options[] = JHtml::_('select.option', '11', 'Lawfirm');
		$options[] = JHtml::_('select.option', '8', 'Super Users');
		$options[] = JHtml::_('select.option', '7', 'Administrator');

		return $options;
	}
	
     /**
	 * Get a list of filter options for Auditor Firm.
	 * @return mixed
	 */
     static function getAuditorFirm()
	{
		$db = JFactory::getDbo();
		$sql = 	'SELECT a.email AS value, a.email AS text' .
			' FROM #__users AS a' .
			' JOIN '.$db->quoteName('#__user_usergroup_map').' AS b ON a.id = b.user_id AND b.group_id = 9';
		...
		{
			$option->text = str_repeat('- ', $option->level).strstr($option->text, '@');
		}
    ...
    /**
	 * Get a list of filter options for Lawfirm Firm.
	 * @return mixed
	 */
     static function getLawFirm()
	{
		$db = JFactory::getDbo();
		$sql = 	'SELECT a.email AS value, a.email AS text' .
			' FROM #__users AS a' .
			' JOIN '.$db->quoteName('#__user_usergroup_map').' AS b ON a.id = b.user_id AND b.group_id = 11';
