...
class UsersModelUsers extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$this->setState('filter.range', $range);
                ...
                $rangegroup = $this->getUserStateFromRequest($this->context.'.filter.rangegroup', 'filter_rangegroup');
		$this->setState('filter.rangegroup', $rangegroup);
		
		$auditor_firm = $this->getUserStateFromRequest($this->context.'.filter.auditorfirm', 'filter_auditorfirm');
		$this->setState('filter.auditorfirm', $auditor_firm);
		
		$law_firm = $this->getUserStateFromRequest($this->context.'.filter.lawfirm', 'filter_lawfirm');
		$this->setState('filter.lawfirm', $law_firm);
	...
	protected function getListQuery()
	{
		...
		if (is_numeric($active))
		{
			...
			elseif ($active == '1')
			{
				$query->where($query->length('a.activation').' = 32');
                	...
                	//get global configuration for user group
		...
		//Filter the items over custom group id
		$custom_groupId = $this->getState('filter.rangegroup');
		if($custom_groupId == $auditor){
			$this->setState('filter.lawfirm', $value = null);
		}elseif($custom_groupId == $lawfirm){
			$this->setState('filter.auditorfirm', $value = null);
		}else{
			$this->setState('filter.lawfirm', $value = null);
			$this->setState('filter.auditorfirm', $value = null);
		}
		if($custom_groupId){
			...
			if ($custom_groupId)
			{
				if($this->getState('filter.auditorfirm')){
					$custom_groupId = $auditor.",".$auditor_emp;
				}elseif($this->getState('filter.lawfirm')){
					$custom_groupId = $lawfirm.",".$lawfirm_emp.",".$lawfirm_partner;
				}
				$query->where('map2.group_id IN('.$custom_groupId.')');
				
			}
		}
		
	    //Filter the items over auditor domain name
	    
		if ($this->getState('filter.auditorfirm') !== '')
		{
			$domain = strstr($this->getState('filter.auditorfirm'),'@');
			$token	= $db->Quote('%'.$db->escape($domain).'%');
			$query->where('a.email LIKE '.$token);
		}
		
	   //Filter the items over lawfirmdomain name
		if ($this->getState('filter.lawfirm') !== '')
		{
			$domain = strstr($this->getState('filter.lawfirm'),'@');
			$token	= $db->Quote('%'.$db->escape($domain).'%');
			$query->where('a.email LIKE '.$token);
	...
	public function getUserDetail($id){
		...
		$query = "SELECT a.*,"
				  ...
				  ." WHERE a.id = b.user_id AND c.lid=a.id AND a.id =".$db->Quote($id);
		...
		$email = $result->email;
		//get groupid
		$query_group = "SELECT group_id FROM #__user_usergroup_map where user_id = ".$db->Quote($id);
		$db->setQuery($query_group);
		$db->query();
		$group_id = $db->loadObject();
		$user_group = $group_id->group_id;
		$fl_user_gp = $group_id->group_id;
		//get offices
		$query_office = "SELECT a.* FROM #__users_office as a where a.lid = ".$db->Quote($id);
		
		$db->setQuery($query_office);
		$db->query();
		$offices = $db->loadObjectlist();
		$allowuser = array($lawfirm,$auditor);
		if(in_array($user_group,$allowuser)){
			//get employee
			//check for audit employee groupid
			if($user_group == $auditor){
				$user_group = $auditor_emp;
			}elseif($user_group == $lawfirm){
				$user_group = "'.$lawfirm_emp.','.$lawfirm_partner.'";
			}
			$employeelist = $this->getEmployee($id);
			$employer = array();
			//get payment info of firm admin
			$payment_data = $this->getpaymentinfo($id);
		}
		else{
			$employeelist = array();
		...
		$userdata = array("profile"=>$result,"offices"=>$offices,'employeelist'=>$employeelist,'employer'=>$employer,'payment'=>$payment_data,'groupid'=>$fl_user_gp);
		return $userdata;
	...
	public function getEmployee($id){
		...
		$db = $this->getDbo();
		$query = "SELECT a.* FROM #__users as a,#__users_profile_detail as b where a.id = b.lid and b.parent = ".$db->Quote($id)."and b.lid != ".$db->Quote($id);
		$db->setQuery($query); 
		$db->query();
	...
	public function getEmployer($id){
		$db = $this->getDbo();
		//get parent id
		//$query = "SELECT a.parent FROM #__users_profile_detail as a where a.lid = ".
		$query = "SELECT a.* FROM #__users as a,#__users_profile_detail as b where a.id = b.parent and b.lid = ".$db->Quote($id);
		$db->setQuery($query); 
		$db->query();
		$data = $db->loadObject();
		return $data;
	...
	public function getpaymentinfo($id){
		$db = $this->getDbo();
		$query = "SELECT a.cc_number,a.name_on_cc,a.address,a.city,a.state,a.country,a.zip,a.esn FROM #__users_payment_detail as a where lid = ".$db->Quote($id);
		$db->setQuery($query);
		$db->query();
		$result = $db->loadObject();
		return $result;
