...
public function login()
	{
		...
		if (true === $app->login($credentials, $options)) { 
			...
			//update the lawconfirm user login count
			$model	= $this->getModel('Login', 'LegalconfirmusersModel');
			$result = $model->addlogincount();
			if(in_array($auditor_emp, $user->groups)){
  ...
  /*
   * code by yogendra for checking the looged in user and redirect to user according to the group.
   * @todo
   */
	public function checkprofile()
	{
		...
		$lawfirm_partner = $config->getValue('config.lawfirm_partner');
		if(in_array($auditor_emp, $user->groups)){
			$app->redirect(JRoute::_('index.php?option=com_legalconfirm&view=userprofile', false));
		}elseif(in_array($auditor, $user->groups)){
			$app->redirect(JRoute::_('index.php?option=com_legalconfirm&view=userprofile', false));
		}elseif(in_array($lawfirm, $user->groups)){ 
			$app->redirect(JRoute::_('index.php?option=com_lawfirm&view=userprofile', false));
		}elseif(in_array($lawfirm_emp, $user->groups)){
			$app->redirect(JRoute::_('index.php?option=com_lawfirm&view=userprofile', false));
		}elseif(in_array($lawfirm_partner, $user->groups)){ 
			$app->redirect(JRoute::_('index.php?option=com_lawfirm&view=userprofile', false));
