 ...
 * @version		1.6
 */
class LegalconfirmusersControllerReset extends LegalconfirmusersController
{
	/**
	...
	public function request()
	{
		...
		$model	= $this->getModel('Reset', 'LegalconfirmusersModel');
		...
		if ($return instanceof Exception) {
			...
			$route	= 'index.php?option=com_legalconfirmusers&view=reset'.$itemid;
			...
			// The request failed.
			// Get the route to the next page.
			//$itemid = LegalconfirmusersHelperRoute::getResetRoute();
			$itemid = $itemid !== null ? '&Itemid='.$itemid : '';
			$route	= 'index.php?option=com_legalconfirmusers&view=reset'.$itemid;
...
$itemid = "";
			$itemid = $itemid !== null ? '&Itemid='.$itemid : '';
			$route	= 'index.php?option=com_legalconfirmusers&view=reset&layout=confirm'.$itemid;
	...
	function confirm()
	{
		...
		if ($return instanceof Exception)
		{
			...
			//$itemid = UsersHelperRoute::getResetRoute();
			$itemid = $itemid !== null ? '&Itemid='.$itemid : '';
			$route	= 'index.php?option=com_legalconfirmusers&view=reset&layout=complete'.$itemid;
	...
	public function complete()
	{
		...
		$app	= JFactory::getApplication();
		$model	= $this->getModel('Reset', 'LegalconfirmusersModel');		$data	= JRequest::getVar('jform', array(), 'post', 'array');
		...
		if ($return instanceof Exception) {
			...
			if($model->getError() == 'COM_USERS_FIELD_RESET_PASSWORD2_MESSAGE')
			{
			 // $model->getError() = 'Password invalid';
			 //COM_USERS_FIELD_RESET_PASSWORD2_MESSAGE="Password invalid"
			 $message = 'Completing reset password failed: Password invalid';
			}
			// Go back to the complete form.
			$this->setRedirect(JRoute::_($route, false), $message, 'notice');
			return false;
