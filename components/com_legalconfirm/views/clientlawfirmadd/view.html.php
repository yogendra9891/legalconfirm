...
/**
 * View class for adding more lawfirms.....
 */
class LegalconfirmViewClientlawfirmadd extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		$this->signer = $this->get('Signer');  
			...
			$this->alreadySelectedLawfirm = $this->get('LawfirmsGettings');

		//finding selected law firms from session .....
		...
		if(JRequest::getVar('templateeditmessage') != '')
		{
			if(JRequest::getVar('templateeditmessage') == 'success'){
				$app->enqueueMessage(JText::_('COM_LEGALCONFIRM_EDIT_TEMPLATE_SUCCESS'), 'message');
			}elseif(JRequest::getVar('templateeditmessage') == 'unsuccess'){
				$app->enqueueMessage(JText::_('COM_LEGALCONFIRM_EDIT_TEMPLATE__ERROR'), 'message');
			}
			$app->redirect(JRoute::_('index.php?option=com_legalconfirm&view=clientslog&id='.$clientid, false));
		}		
		parent::display($tpl);
	}
