...
class LawfirmModelLawfirmpartner extends JModelList {
	...
	public function getAssignedPinnedTask(){
		...
		$query1 = "SELECT a.*,b.partner_id
		           ...
		           ON a.id = b.aid 
		           WHERE b.partner_id =".$userId." AND b.lid !=".$userId;
	...
	public function sendProposalToAuditor($aid){
		//get db object
		$db = $this->getDBO();
		$query = "UPDATE #__lawfirm_assignproposal as a SET a.taskstatus = '1' WHERE a.id = ".$aid;
		$db->setQuery($query);
		if(!$db->query()){
			return false;
		}
	...
	public function disApprove($aid){
		...
		$query2 = "SELECT a.id  FROM #__lawfirm_disapproved_tasks as a WHERE a.aid = ".$aid;
		$db->setQuery($query2);
		$db->query();
		$result = $db->loadObject();
