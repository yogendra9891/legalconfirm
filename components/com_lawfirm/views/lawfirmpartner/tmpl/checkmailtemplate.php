...
$aptype = JRequest::getVar('aptype');
?>

<?php if($aptype == "1"){ ?>
<script>
var aaid = "<?php echo $aid;?>";
var vb = window.parent.document.getElementById('changeactive'+aaid).innerHTML='Yes';
var vb1 = window.parent.document.getElementById('approved'+aaid).value='1';
</script>
<?php } ?>
<script>
//Method to validate the form
function validateFormapprove(formId)
{
...
function validateFormForPdf(formId){
	 var formId=document.getElementById(formId);
	 if(validate(formId))
	       {
	          ...
	          //this check triggers the validations
	    	   ...
	    	   var pdfval = $('#pdf').val();
			    if(pdfval == ""){
			    	$('#pdferr').html('Please select PDF file');
			    }else{
			    $('#pdferr').html('Invalid Pdf');
			    }
			    return false;
	       }
}
...
function validateFormsavetopartner(formId)
{
	 ...
	 if(check_id == "0"){
		$.msgBox({
			    title:"Alert",
			    content:"Please first approve the template."
			});
	  return false;
      }else{
 ...
 if(validate(formId))
	...
	$.msgBox({
	title: "Are You Sure",
	content: "Template would be saved and send to auditor?",
	type: "confirm",
...
$content = $this->getMailTemplateByLawfirm($aid);
$id = $content->id;
...
//check for pdf or custom template
...
if($template_type=="pdf"){
 ...
 if($taskstatus == 1){
 	?>
 	<h4><?php if($taskstatus == 1){echo "Already Submitted to Auditor";}else{"Submit to Auditor.";}?></h4>
 	<div class="pdfwidth">
...
<!-- Pdf uploaded by employee and submitted for approval -->
<form method="post" name="register2" id="register2" action="index.php" enctype="multipart/form-data">
...
<input type="hidden" id="pdfname" name="pdfname" value="<?php echo $pdf_name;?>"/>
<!-- check for if propsal is already submitted to partner for proposal-->

<input class="button" type="submit" value="Approve" name="approve" onclick="return validateFormapprove('register2');"/> 
<span class="disapprovelink"><a href="<?php echo JRoute::_('index.php?option=com_lawfirm&view=lawfirmpartner&task=lawfirmpartner.disApprove&aid='.$aid);?>" class="button disbutton">Disapprove</a></span>
<input class="button" type="submit" value="Send Response to Auditor" name="save" onclick="return validateFormsavetopartner('register2');"/> 
...
<input type="hidden" name="task" value="lawfirmpartner.approvepdf" /> 
...
<div style="text-align:center;"><p>Or <br />Upload new pdf</p></div>
<!-- Pdf uploaded by partner and submitted for approval -->
...
<!--<input class="button" type="submit" value="Approve" name="approve" onclick="return validateFormapprove('register1');"/> -->
<input class="button" type="submit" value="Send Response to Auditor" name="save" onclick="return validateFormForPdf('register1');"/> 
...
<input type="hidden" name="aid" value="<?php echo $aid; ?>" /> 
<input type="hidden" name="type" value="custom" /> 
<input type="hidden" name="task" value="lawfirmpartner.uploadNewPdf" /> 
<?php echo JHtml::_( 'form.token' ); ?>
</form>
<?php 
 ...
 $temp1 =  $content->custom_template;
?>
<div id="mailtemplate">
<h4><?php if($taskstatus == 1){echo "Already Submitted to Auditor";}else{"Edit the Template";}?></h4>
<form method="post" name="register1" id="register1" action="index.php" onsubmit="return validateForm('register1');"><?php 
...
echo $editor->display( 'full_text', $temp1 ,  '100%', '400', '80', '15' );
?> 
...
<input class="button" type="submit" value="Approve" name="approve" onclick="return validateFormapprove('register1');"/>
<span class="disapprovelink"><a href="<?php echo JRoute::_('index.php?option=com_lawfirm&view=lawfirmpartner&task=lawfirmpartner.disApprove&aid='.$aid);?>" class="button disbutton">Disapprove</a></span>
<input class="button" type="submit" value="Send Response to Auditor" name="save" onclick="return validateFormsavetopartner('register1');"/> 
<?php } ?>
</div>
<input type = "hidden" id="isapprove" value="<?php echo $is_approve_bypartner;?>"/>
<input type="hidden" name="option" value="com_lawfirm" /> 
...
<input type="hidden" name="aid" value="<?php echo $aid; ?>" /> 
<input type="hidden" name="type" value="custom" /> 
<input type="hidden" name="task" value="lawfirmpartner.addcustomhtml" /> 
<?php echo JHtml::_( 'form.token' ); ?>
</form>
</div>
<?php } ?></div>
