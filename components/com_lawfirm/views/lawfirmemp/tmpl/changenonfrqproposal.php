...
<span class="attorney-list-tasks-title">You can add the more clients to the assigned
list from here</span>
<span class="attorney-list-tasks">
<a
	href="<?php echo JRoute::_('index.php?option=com_lawfirm&view=lawfirmemp&layout=lawfirmemptask');?>"><span style="position:relative;top:-6px;">Assigned Tasks</span><img src='<?php echo JURI::root()."templates/legalconfirm/images/tasks.png"?>' /></a>
</span>
</div>
...
<form name="adminForm" method="post"
	action="index.php?option=com_lawfirm&view=lawfirmemp&task=lawfirmemp.notfrequentedit">
<table class="alllist">
	<tr class="formadmin">
...
<?php echo JHtml::_('grid.sort', 'COM_LAWFIRM_EMPLOYEE_DATE_INITIATED_AUDITOR', 'a.id', $this->state->get('list.direction'),$this->state->get('list.ordering') ); ?>
	...
	<tr id="row<?php echo $single->id;?>">
		...
		<td><?php if($single->is_pinbyemp == '0'){?> <span class="pin2"
			id="pin<?php echo $single->id;?>"><input style="cursor: pointer;"
			type="button"
			onclick="pintask(<?php echo $single->id ;?>,<?php echo $single->pid ;?>);"
			value="Click to assign" /></span> <span class="unpin2"
			id="unpin<?php echo $single->id;?>"><input style="cursor: pointer;"
			type="button"
			onclick="unpintask(<?php echo $single->id ;?>,<?php echo $single->pid ;?>);"
			value="Click to unassign" /></span> <?php } else {?> <span
			class="unpin" id="unpin<?php echo $single->id;?>"><input
			style="cursor: pointer;" type="button"
			onclick="unpintask(<?php echo $single->id ;?>,<?php echo $single->pid ;?>,<?php echo $single->aid ;?>);"
			value="Click to unassign" /></span> <?php } ?></td>
	</tr>
