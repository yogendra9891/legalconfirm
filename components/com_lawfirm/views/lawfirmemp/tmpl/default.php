...
foreach($this->items as $single){
	...
	<?php }elseif($single->is_pinbyemp =='1' && $single->emp_id != $userId){
		...
		?>
	    <p style="color:#9B9B9B; font-weight:bold; text-align: left;"><?php echo "Assigned By ".$lawuser->name; ?></p>
	<?php }elseif($single->is_pinbyemp =='1' && $single->emp_id == $userId && $single->is_readybyemp =='1'){
		
		?>
		  <p style="color:#9B9B9B; font-weight:bold; text-align: left;"><?php echo "Submitted to Partner"; ?></p>
		<?php
	}else{
	?>
