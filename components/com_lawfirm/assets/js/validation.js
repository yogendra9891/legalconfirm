 ...
 function validate(formId)
{
	 ...
	 var flag=true;
	 	for(var i=0;i<formId.elements.length;i++)
	 	{
			...
			else if(formId.elements[i].className=="required pdf")
			{
				
				var fileInsert = document.getElementById('pdf').value;
				
				if(fileInsert != ""){
				  filear = fileInsert.split('.').pop().toLowerCase();
				
				  if($.inArray(filear, ['pdf']) == -1) {
			...
			else if(formId.elements[i].className=="zip required")
			{
				...
				// alert(
				// (/(^\d{5}$)|(^\d{5}-\d{4}$)/).test(formId.elements[i].value));
				if(isEmpty(formId.elements[i].value))
	 			{
	 				flag=false;
	 				formId.elements[i].style.border="2px solid red";
			...
			else if(formId.elements[i].className=="ccnumber required")
			{
				...
				if((american_regex).test(formId.elements[i].value)){
					$('#errccnumber').html('');
				}
			    else if(visa_regex.test(formId.elements[i].value)){
			    	$('#errccnumber').html('');
				}
				else if(master_regex.test(formId.elements[i].value)){
					$('#errccnumber').html('');
				}
				else if(discover_regex.test(formId.elements[i].value)){
					$('#errccnumber').html('');
				}
				else if(dinner_regex.test(formId.elements[i].value)){
					$('#errccnumber').html('');
				}
				else if(jcb_regex.test(formId.elements[i].value)){
					
				}
				else if(mastero_regex.test(formId.elements[i].value)){
					$('#errccnumber').html('');
				}
				else{
					flag=false;
